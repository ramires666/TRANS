...
Usage:
    ...
    python -m app.cli --in file.pdf --out _EN.pdf --target-lang en --resume
...
from pipeline.config.loader import (configure_target_language, ensure_dirs,
...
def main() -> None:
    ...
    ap_cli.add_argument(
        "--target-lang", dest="target_lang",
        choices=("ru", "en", "de", "fr", "es", "ja"),
        help="переопределить язык результата для текущего запуска",
    )
    ...
    if args.target_lang:
        configure_target_language(cfg, args.target_lang)
    ensure_dirs(cfg)
